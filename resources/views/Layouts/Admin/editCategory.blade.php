...
<body>
    <div
        ...
        <div class="h-full ml-14 mt-14 mb-10 md:ml-64">
            <div class="mt-4 mx-4">
                <form class="mt-8 space-y-6 p-7 rounded-md bg-white dark:bg-orange-400 text-black dark:text-white"  action="{{ route('category.update', $category->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="flex w-full gap-5">
                        <label for="UserEmail"
                            ...
                            <input type="text" id="UserEmail" placeholder="Category Name" name="cname"  value="{{$category->name}}"
                    ...
                    <div class="relative">
                        <label for="UserEmail"
                           ...
                           <textarea class="peer h-8 w-full border-none bg-transparent p-0 placeholder-transparent focus:border-transparent focus:outline-none focus:ring-0 dark:text-white sm:text-sm" name="description" id="" cols="30" rows="4" >{{ $category->description }}</textarea>
                        </label>
                    </div>
                    <div>
                        <button type="submit"
                            class="block w-full text-white bg-orange-400 hover:bg-orange-900 focus:ring-4 focus:ring-orange-400 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 sm:mr-2 lg:mr-0 dark:bg-orange-400 dark:hover:bg-orange-900 focus:outline-none dark:focus:ring-purple-800 p-4 transition hover:scale-105">Update
                        </button>
                    </div>
